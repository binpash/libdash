...
and string_of_arg_char ?quote_mode:(quote_mode=Unquoted) = function
  | E c ->
     let chars_to_escape = "'\"`(){}$!&|;" in
     let chars_to_escape_when_no_quotes = "*?[]#<>~ " in
     if String.contains chars_to_escape c
     then "\\" ^ String.make 1 c
     else if String.contains chars_to_escape_when_no_quotes c && quote_mode=Unquoted
     then "\\" ^ String.make 1 c
     else Char.escaped c
  | C '"' when quote_mode=Quoted -> "\\\""
  | C c -> String.make 1 c
  | T None -> "~"
  | T (Some u) -> "~" ^ u
  ...
  | Q a -> "\"" ^ string_of_arg ~quote_mode:Quoted a ^ "\""
  | B t -> "$(" ^ to_string t ^ ")"

and string_of_arg ?quote_mode:(quote_mode=Unquoted) = function
  | [] -> ""
  | c :: a ->
...
and string_of_redir = function
  ...
  | Heredoc (t,fd,a) ->
     let heredoc = string_of_arg ~quote_mode:Heredoc a in
