...
module Types (F : Ctypes.TYPE) = struct
  ...
  module Stackmark = struct
    ...
    let t : stackmark structure typ = structure "stackmark"
    let stackp = field t "stackp" (ptr void)
    let nxt = field t "stacknxt" string
    let size = field t "stacknleft" F.size_t
    let () = seal t
  end
